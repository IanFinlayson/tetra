 ...
 * Although the methods are presently rtemplated, becuase it is an error to
 * return something from a parallel segment
 * these methods could technically be untemplated and just have an int thrown as
 * the datatype for the basic evaluateNode
 */
...
void dummy(void* args) { UNUSED(args) }

// This class wraps the arguments which must be passed to an evaluateNode
// statment so they can be passed as arguments to spawned threads
template <typename T>
struct evalArgs {
  const Node* node;
  TData<T>& ret;
  ...
  evalArgs(const Node* pNode, TData<T>& pRet, scope_ptr pScope,
           scope_ptr* pGlobal, TetraContext* pContext)
      : node(pNode),
        ret(pRet),
        scope(pScope),
        globalScope(pGlobal),
        debugCarrier(pContext) {
    //	cout << "This gets called!"<<endl;//do nothing
  }
};

// This class wraps the arguments that must be passed to a worker thread for a
// parallel for loop
template <typename T>
struct evalForArgs {
  evalArgs<T>* args_ptr;
...
void wrapEvaluation(void* args) {
  ...
  if (TetraEnvironment::isDebugMode()) {
    // std::cout << "Parallel for-loop thread created: " <<
    // contextCopy.getThreadID();
    TetraEnvironment::getObserver().threadCreated_E(contextCopy.getThreadID(),
                                                    contextCopy);
  ...
  try {
    ...
    cout << "The following error was encountered while executing a thread:"
         << endl;
    contextCopy.printStackTrace();

    cout << e << endl;
  } catch (std::exception e) {
    cout << "A standard exception was encountered while executing a thread: "
         << endl;
    cout << e.what() << endl;
  } catch (...) {
  ...
  // Although the memory is allocated in a different thread, that thread might
  // terminate before the new thread
  // As a result, it must be deleted when this thread terminates
  // This will need to be changed later to handle when this thread terminates
  // unexpectedly

  // ThreadEnvironment::removeThread(pthread_self());

  if (TetraEnvironment::isDebugMode()) {
    TetraEnvironment::getObserver().threadDestroyed_E(
        contextCopy.getThreadID());
  }
...
// perform a given task as per a for loop
template <typename T>
void wrapMultiEvaluation(void* args) {
  ...
  while (*(argList.countVal_ptr) < /*arraySize*/ argList.values_ptr->size()) {
    ...
    (*(argList.args_ptr->scope)
          ->template lookupVar<T>(*(argList.varName_ptr))) =
        *static_cast<T*>(
    ...
    try {
      ...
      contextCopy.normalizeStatus();
      evaluateNode<T>(argList.args_ptr->node, argList.args_ptr->ret,
                      contextCopy);
    } catch (Error e) {
    ...
    // Lock the mutex before checking the condiiton again and overwriting the
    // value
    ////TODO put this back in
    pthread_mutex_lock(argList.count_mutex_ptr);

    // If the partallel for loopencountered a break statement, make it so that
...
// The varName must be passed so the thread can initialize its own copy of the
// variable
template <typename T>
...
void evaluateParallel(const Node* node, TData<T>& ret, TetraContext& context) {
  switch (node->kind()) {
    case NODE_PARFOR: {
      ...
      // variables
      if (TetraEnvironment::isDebugMode()) {
        TetraEnvironment::getObserver().notifyThreadSpecificVariable_E(
            node->child(0)->getString());
        context.registerParallelForVariable(node->child(0)->getString());
        // TODO "l v" in debugger does not currently see parallel for variables
        // on initialization
      }

      int currentIteration =
          0;  // Used to determine which iteratio nshould be tackled next
      pthread_mutex_t iter_mutex;
      pthread_mutex_init(&iter_mutex, NULL);

      // Locking this mutex here may in fact be unnecessary
      // However, helgrind complains if this is not here, and this thread onl;y
      // holds the lock until all the threads are created
    ...
    case NODE_PARALLEL: {
      ...
      // Note that it is an error to encounter some type of flag statement
      // within a parallel block
      // i.e. breaks/continues/returns will be ignored if their change in
      // control would bring them outside the parallel block
    ...
    case NODE_LOCK: {
      // Get the mutex associated with this lock. or create a new one if it does
      // not exist
      pthread_mutex_t* lock =
          ThreadEnvironment::identifyMutex(node->child(0)->getString());
