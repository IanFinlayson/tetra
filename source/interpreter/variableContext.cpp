...
VarTable::~VarTable() { pthread_rwlock_destroy(&table_mutex); }
...
std::list<std::pair<pthread_t, TData<void*> >, gc_allocator<std::pair<pthread_t, TData<void*> > > >& VarTable::declareParForVar(
  ...
  // This could happen if, say, someone declares a parfor within a loop, in
  // which case we should append to that array
  if (std::find_if(parForVars.begin(), parForVars.end(), CheckName(varName)) !=
      parForVars.end()) {
    // return
  ...
  // If not found, we will have to release the mutex anyways before trying to
  ...
  // a ParFor in this scope
  // pthread_mutex_lock(&table_mutex);
  pthread_rwlock_wrlock(&table_mutex);

  // While we were waiting for the lock, another thread may have pushed this
  // variable into the list!
  if (std::find_if(parForVars.begin(), parForVars.end(), CheckName(varName)) !=
      parForVars.end()) {
    // Release the mutex before returning
    pthread_rwlock_unlock(&table_mutex);
    return std::find_if(parForVars.begin(), parForVars.end(),
                        CheckName(varName))
        ->second;
  }
...
// i.e. Adding in the varName does not cause any new objects to be created
// (unlike lookupVar which creates one if it does not yet exist)
// Also, the data is not marked as deletable, so the referenced data will not be
// destroyed when this data goes out of scope
// Used primarily for passing vectors to functions/for-loops
// Returns the reference once it creates it, so it can be set to reference the
// proper thing
/*
   TData<void*>& VarTable::declareReference(const string varName) {

//Sets the data for what we will insert as the location of the actual data being
referenced
TData<void*> inserter;
//Note that setDeletableType is not called, because this object going out of
scope should not result in the data getting deleted
