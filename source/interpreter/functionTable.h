 ...
 *Since there is only one funciton table per program (even if using multiple
 ...
 file's syntax tree) There can only be one instance.
 * TODO: For a long time, this class has been a convoluted mix between a
 * singleton and static member class. Must query the design plans of the
 * debugger to see whether this will ever be needed as an object, so we can
