...
VirtualConsole& ConsoleArray::getSpecifiedConsole(int thread,
                                                  bool debug) const {
...
void ConsoleArray::releaseConsoleMutex(int thread, bool debug) const {
  pthread_mutex_unlock(
      consoleMutexes[invokeConsolePolicy(thread, debug)].first);
}

void ConsoleArray::waitOnCondition(int thread, bool debug) const {
  pthread_cond_wait(consoleMutexes[invokeConsolePolicy(thread, debug)].second,
                    consoleMutexes[invokeConsolePolicy(thread, debug)].first);
}

void ConsoleArray::broadcastCondition(int thread, bool debug) const {
  pthread_cond_broadcast(
      consoleMutexes[invokeConsolePolicy(thread, debug)].second);
}
