...
#include <QtWidgets>
/*Debug Windows are created during the debugging of a Tetra program and represent single threads in the program*/
...
void DebugWindow::updateLineNumberArea(const QRect &rect, int dy){
    if (dy)
        ...
        debuggerLineNumberArea->update(0, rect.y(), debuggerLineNumberArea->width(), rect.height());
...
void DebugWindow::resizeEvent(QResizeEvent *e){
    ...
    debuggerLineNumberArea->setGeometry(QRect(cr.left(), cr.top(), lineNumberAreaWidth(), cr.height()));
}
...
void DebugWindow::lineNumberAreaPaintEvent(QPaintEvent *event){
    ...
    while (block.isValid() && top <= event->rect().bottom()) {
        if (block.isVisible() && bottom >= event->rect().top()) {
            ...
            painter.drawText(0, top, debuggerLineNumberArea->width(), fontMetrics().height(),
...
bool DebugWindow::checkLineHighlighted(){
    return lineHighlighted;
}


void DebugWindow::setTabWidth(int tabWidth){
    this->tabWidth = tabWidth;
}

int DebugWindow::getTabWidth(){
    return this->tabWidth;
}

//fills DebugWindow with text
void DebugWindow::fill(QString text){
