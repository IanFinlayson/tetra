...
std::map<tstring, ClassContext> classes;
...
void ClassContext::addMembers(Node* node) {
    ...
    if (node->kind() == NODE_IDENTIFIER) {
        this->addMember(Symbol(node->getStrval(), node->type(), node->getLine()));
...
std::map<tstring, Node*> ClassContext::removeInits() {
...
DataType* inferRead(Node* funcall) {
    ...
    if (funcall->numChildren() > 1) {
        throw Error(funcall->getStrval() + " should not have any parameters", funcall->getLine());
    }

    /* get the return type right */
    if (funcall->child(0)->getStrval() == "read_string")
        return new DataType(TYPE_STRING);
    if (funcall->child(0)->getStrval() == "read_int")
        return new DataType(TYPE_INT);
    if (funcall->child(0)->getStrval() == "read_real")
        return new DataType(TYPE_REAL);
    if (funcall->child(0)->getStrval() == "read_bool")
...
bool inPar(Node* node) {
    ...
    while (curr->kind() != NODE_FUNCTION) {
         ...
         * block of any kind */
        if (curr->kind() == NODE_PARFOR || curr->kind() == NODE_PARALLEL ||
...
bool inLoop(Node* node) {
    ...
    while (curr->kind() != NODE_FUNCTION) {
        /* check to see if we have found a containing parallel
         * block of any kind */
        if (curr->kind() == NODE_PARFOR || curr->kind() == NODE_FOR || curr->kind() == NODE_WHILE) {
            return true;
        }
        /* go up one node */
        curr = curr->getParent();
...
bool checkReturns(Node* node) {
    switch (node->kind()) {
        ...
        case NODE_IF:
            ...
            return node->numChildren() == 3 && checkReturns(node->child(1)) &&
                   checkReturns(node->child(2));

        case NODE_ELIF:
            /* returns true if there is and else
             * and all the conditional paths return*/
            return node->numChildren() == 3 && checkReturns(node->child(0)) &&
...
Symbol* findIdSym(Node* expr, Node* func = NULL) {
    ...
    while (lambda && !found) {
        ...
        if (lambda->hasSymbol(expr->getStrval())) {
            *sym = lambda->lookupSymbol(expr->getStrval(), lambda->getLine());
    ...
    if (func && func->hasSymbol(expr->getStrval())) {
        ...
        *sym = globals[expr->getStrval()];
        ...
        /* look it up */
        *sym = Symbol(expr->getStrval(), functions.getFunctionsNamed(expr->getStrval()),
                      expr->getLine(), true);
    }
...
DataType* inferExpressionPrime(Node* expr, Node* func) {
    ...
    switch (expr->kind()) {
        case NODE_ASSIGN: {
            ...
            if (expr->child(0)->kind() == NODE_IDENTIFIER) {
                ...
                if (!sym && !rhs->isEmptyContainerType()) {
                    ...
                    lhs = rhs;
                    func->insertSymbol(
                        *new Symbol(expr->child(0)->getStrval(), lhs, expr->child(0)->getLine()));
        ...
        case NODE_FUNCALL: {
             ...
             */
            throw Error("No matching function for: '" + expr->child(0)->getStrval() +
        ...
        case NODE_IDENTIFIER: { /* first check if it already has a type (this
            ...
            if (expr->type()) {
                ...
                if (func->hasSymbol(expr->getStrval())) {
                    /* complain! */
                    throw Error("The identifier '" + expr->getStrval() + "' already exists.",
                                expr->getLine());
                }

                /* if we make it here, we need to add this declaration to this
                 * function's sym table */
                func->insertSymbol(*new Symbol(expr->getStrval(), expr->type(), expr->getLine()));
            ...
            if (!sym) {
                /* complain! */
                throw Error("Reference to non-existent identifier '" + expr->getStrval() + "'.",
                            expr->getLine());
            }
        ...
        case NODE_DOT: {
            ...
            } else if (!classes[*lhs->className].hasMember(expr->child(1)->getStrval())) {
        ...
        case NODE_METHOD_CALL: {
            ...
            } else if (!classes[*lhs->className].hasMethod(rhsParams,
                                                           expr->child(1)->child(0)->getStrval())) {
                throw Error("Class '" + *(lhs->className) + "' does not contain method '" +
                                expr->child(1)->child(0)->getStrval() + "'. " +
                                expr->child(1)->getStrval() + typeToString(rhsParams),
                            expr->getLine());
            }

            /* return the return type of the method */
            return classes[*lhs->className]
                .getMethod(rhsParams, expr->child(1)->child(0)->getStrval())
                ->type();
        }
...
void checkMuTasks(Node* block, Node* func) {
    ...
    if (block->child(1) || block->kind() == NODE_WAIT) {
        ...
        if (!sym && block->kind() != NODE_WAIT) {
            ...
            sym = new Symbol(block->child(0)->getStrval(), new DataType(kind),
...
void inferParams(Node* node, Node* func) {
    ...
    /* else if it's just one param, handle it */
        ...
        func->insertSymbol(Symbol(node->getStrval(), node->type(), node->getLine()));
...
void inferGlobal(Node* node, bool isConst = false) {
    ...
    if (globals.count(node->child(0)->getStrval()) > 0 ||
        ...
        throw Error(varType + " '" + node->child(0)->getStrval() + "' has been defined.");
    }
...
void addStls() {
    tstring stls[] = {"len", "read_string", "read_int", "read_real", "read_bool", "print"};

    for (unsigned long i = 0; i < sizeof(stls) / sizeof(stls[0]); i++) {
        /* add them to the globals */
        DataType type(TYPE_FUNCTION);

        globals.insert(std::pair<tstring, Symbol>("len", Symbol(stls[i], &type, 0, true)));
    }
}
...
void addMembers(ClassContext* context, Node* node) {
    ...
    if (node->kind() == NODE_IDENTIFIER) {
        /* add it */
        context->addMember(Symbol(node->getStrval(), node->type(), node->getLine()));
    } else if (node->kind() == NODE_CLASS_PART) {
        /* recursively add other parts */
...
void initSquared(ClassContext context) {
    ...
    std::map<tstring, Node*> inits = context.removeInits();

    DataType* type = new DataType(TYPE_CLASS);
    *(type->className) = context.getName();

    /* loop through any inits*/
    for (std::map<tstring, Node*>::iterator it = inits.begin(); it != inits.end(); it++) {
        ...
        functions.insert(std::pair<tstring, Node*>(
...
void inferFunction(Node* node) {
    ...
    if (globals.count(node->getStrval()) || classes.count(node->getStrval())) {
...
void inferClass(Node* node) {
    if (node->kind() == NODE_CLASS) {
        ...
        if (globals.count(node->getStrval()) || classes.count(node->getStrval()) ||
            ...
            throw Error(
                "Member identifier '" + node->getStrval() + "' already exists in current scope.",
                node->getLine());
        }
